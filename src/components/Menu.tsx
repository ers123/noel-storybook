            ...
            {isOpen && (
                <>
                    ...
                    <motion.div
                        ...
                        className="fixed top-0 left-0 bottom-0 w-80 md:w-96 bg-white z-50 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.3)] flex flex-col"
                    >
                        <div className="p-5 sm:p-6 md:p-7 flex items-center justify-between bg-gradient-to-br from-indigo-500 via-indigo-600 to-purple-600 relative overflow-hidden">
                            <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,')] opacity-20"></div>
                            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-2 sm:gap-3 relative z-10">
                                <div className="p-1.5 sm:p-2 bg-white/20 rounded-xl backdrop-blur-sm">
                                    <BookOpen className="w-5 h-5 sm:w-6 sm:h-6" />
                                </div>
                                {language === 'ko' ? '목차' : 'Contents'}
                            </h2>
                            <button onClick={onClose} className="p-2.5 sm:p-3 hover:bg-white/20 rounded-xl transition-all hover:scale-110 active:scale-95 relative z-10 backdrop-blur-sm min-h-[48px] min-w-[48px]">
                        ...
                        <div className="flex-1 overflow-y-auto py-2">
                            {story.map((chapter, index) => (
                                <button
                                    ...
                                    className={`group w-full text-left px-4 sm:px-5 py-4 sm:py-5 transition-all duration-300 border-b border-gray-100/50 last:border-0 relative min-h-[64px]
                    ${chapter.id === currentChapterId
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-3 sm:gap-4 ml-1">
                                        <div className={`flex-shrink-0 w-9 h-9 sm:w-10 sm:h-10 rounded-lg flex items-center justify-center font-bold text-sm sm:text-base transition-all duration-300 ${
                                            chapter.id === currentChapterId
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className={`text-xs sm:text-sm font-semibold uppercase tracking-wider mb-1 transition-colors ${
